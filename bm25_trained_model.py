...
# 載入模型 (with QLoRA adapter)
BASE_MODEL_ID = "meta-llama/Llama-3.1-8B-Instruct"
ADAPTER_PATH = "/home/guest/b11902155/ntu_rag/NTU_Assistant_RAG/continual_pretraining/outputs/conti-llama-qlora"
...
if MODEL is None or TOKENIZER is None:
    print("Loading model and tokenizer with QLoRA adapter (this may take a few minutes the first time)...")
    
    # Prefer tokenizer from adapter if available, else from base
    tokenizer_source = ADAPTER_PATH if os.path.exists(os.path.join(ADAPTER_PATH, "tokenizer.json")) else BASE_MODEL_ID
    ...
    base_model = AutoModelForCausalLM.from_pretrained(BASE_MODEL_ID, device_map=device_map, **load_kwargs)
    
    # Attach PEFT LoRA adapter
    if PeftModel is not None:
        MODEL = PeftModel.from_pretrained(base_model, ADAPTER_PATH, torch_dtype=dtype)
        MODEL.eval()
        try:
            MODEL.to(device)
        except Exception:
